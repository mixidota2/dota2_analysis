...
class pub_task(luigi.Task):
    ...
    dotapatch = luigi.Parameter()
    increment_num = luigi.IntParameter(default=0)
    resources = {"pub_task":1}
    ...
    def run(self):
        ...
        while num_of_docs < 10000000:
            current_id = first_id + num_of_docs
            print(f"docs count :{num_of_docs}")
            search_id = current_id + self.increment_num
            self.increment_num += 1
            print(f"increment_num:{self.increment_num}")
            print(f"search id:{search_id}")
            #sleep(1)
            yield get_ranked(matchid=str(search_id),dotapatch=self.dotapatch)
...
if __name__ == '__main__':
    ...
    cred = credentials.Certificate(CRED_PATH)
    firebase_admin.initialize_app(cred)
    luigi.build([pub_task(startid=5167514277,dotapatch="7.23e")],workers=1)
